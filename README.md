...
打开 DSH → **设置 → 邮件通知**。面板分两级，**平时只需要填两个框**：

> **⚠️ 先看这一条**：0.4.0 起，**发不发邮件由一个按钮说了算**，而且默认是**关**的
> —— 装了新版没点过按钮之前，一封通知邮件都不会发。按钮在**会话页头最右侧**：
> 「离开模式：关」→ 点一下变成「离开模式：开」，之后有通知就发到你邮箱（不管你在不在看界面）；
> 再点一下又回到不发。想恢复旧的"离开电脑就自动发"，到高级设置里打开**自动模式**。
...
三个按钮：**保存设置**（写回 `config.json` 并立即生效，不用重启）、**发送测试邮件**（用界面上的草稿试发，没保存也能试；
...
| 谁 | 负责什么 |
| --- | --- |
| **Vergil-long** | 需求、设计取舍、逐轮验收 |
| **DeepSeek Harness**（`deepseek-v4-flash` 智能体） | 代码实现、自检用例、文档 |

本项目是**人与 AI 结对完成**的：做什么、怎么取舍、哪一版算过关，由人决定；
编码、测试与文档由 AI 写。全部代码经过 8 套自检（`npm test`，含"真 cordis + 真 webServer"的
集成测试），关键行为都在真机上实测过 —— 离开模式开关的两个方向、三种通知的实际投递、
以及桌面外壳误报补丁的前后对比。
